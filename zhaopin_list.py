...
if __name__ == '__main__':
    ...
    while 1:
        ...
        if rows.isdigit():
            ...
            print("\033[0;31;47m\t####### 请输入纯数字！！！#######\033[0m\n")
    if choose == str(1):
        filename = time.strftime('[qc-' + str(rows) + '人]' + "%Y-%m-%d %H-%M-%S", time.localtime()) + '.csv'
    elif choose == str(2):
        filename = time.strftime('[qa-' + str(rows) + '人]' + "%Y-%m-%d %H-%M-%S", time.localtime()) + '.csv'
    out = open(filename, 'a+', newline='')
    csv_write = csv.writer(out, dialect='excel')
    csv_header = ['更新时间', '姓名', '工作年限', '年龄', '现居住地', '期望工作地点', '学历', '毕业学校', '专业', '期望月薪', '目前状况',
                  '期望从事职业', '最近所在公司', '最近所在公司职位', '最近工作描述', '链接']
    ...
    while 1:
        try:
            ...
            for n in range(0, int(math.ceil(int(rows)/30))):
                search_result(start_page=n, at_code=at_code, choose=choose)
